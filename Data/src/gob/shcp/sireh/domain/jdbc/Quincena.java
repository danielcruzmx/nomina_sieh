...
public class Quincena implements Domain, Serializable {
    ...
    public Quincena(java.util.Date fecha) {
        int dia;
    ...
    public int diferenciaQuincenas(Quincena qna) {
        int ret = 0;
    ...
    public Quincena sumaQuincenas(int ctas) {
        ...
        Quincena oQna = new Quincena(Integer.parseInt(integerFormat(nAnios, "0000") + integerFormat(nQna, "00")));
        return oQna;
    }

    public List<Quincena> listaDesde(Quincena qnaDesde) {
        int dif = 0;
        int q = 0;
        ...
        lQnas.add(qnaDesde);
        for (q = 1; q <= dif; q++) {
            Quincena qna = qnaDesde.sumaQuincenas(q);
            lQnas.add(qna);
        }
        return lQnas;
    }
