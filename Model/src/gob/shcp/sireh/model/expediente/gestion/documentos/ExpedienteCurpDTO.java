...
public class ExpedienteCurpDTO extends FsnResource implements Model {
    ...
    // JCR
    public ExpedienteCurpDTO() {
        super(NODE_TYPE);
        setVersionable(true);
    }
    public ExpedienteCurpDTO(Map properties) {
    ...
    // Validaciones
    @ValidationMethod(forProperty = "curp", errorCode = "not.null")
    public boolean validateCurpRequired() {
        if (this.isRegistrar()) {
            return (this.getCurp() != null && this.getCurp().length() > 0);
        }
        return true;
    }

    @ValidationMethod(forProperty = "folio", errorCode = "not.null")
    public boolean validateFolioRequiered() {
        if (this.isRegistrar()) {
