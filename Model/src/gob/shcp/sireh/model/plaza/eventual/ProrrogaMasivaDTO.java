...
public class ProrrogaMasivaDTO implements java.io.Serializable, Model{
    public ProrrogaMasivaDTO() {
    }
    ...
    // Tipo movimiento not null
    @ValidationMethod(forProperty = "tipoMovimiento", errorCode = "not.null")
    private boolean validateTipoMovimientoNotNull() {
        if (this.isProrroga()) {
            if (this.getTipoMovimiento() == null || this.getTipoMovimiento().toString().equals("") ) {
                return false;
            }
        }            
        return true;
    ...
    // fechaInicioVigencia not null
    @ValidationMethod(forProperty = "fechaInicioVigencia", errorCode = "not.null")
    private boolean validateFechaInicioVigenciaNotNull() {
        if (this.isProrroga()) {
    ...
    // fechaTerminoVigencia > Fecha de término de contrato
    ...
    private boolean validateFechaTerminoVigenciaVsTerminoContrato() {
        ...
        if (this.isProrroga()) {
            ...
            if (!DateUtils.truncate(this.getFechaTerminoVigencia(), Calendar.DATE).after(DateUtils.truncate(fechaFinCont, Calendar.DATE))) {
                return false;
            }
        }
        return true;
    ...
    //fechaInicioVigencia > Fecha fin de contrato anterior y < Fecha fin de contrato actual
    @ValidationMethod(forProperty = "fechaInicioVigencia", errorCode = "fecha.invalida.rango")
    private boolean validateFechaInicioVigencia() {

        if (this.isProrroga()) {
            if (this.getFechaInicioVigencia() == null || this.getFechaTerminoVigencia() == null) {
                return false;
            }
            ...
            if (!DateUtils.truncate(this.getFechaInicioVigencia(), Calendar.DATE).after(DateUtils.truncate(fechaFinCont, Calendar.DATE)) ||
                !DateUtils.truncate(this.getFechaInicioVigencia(), Calendar.DATE).before(DateUtils.truncate(this.getFechaTerminoVigencia(), Calendar.DATE))) {
                return false;
            }
        }
        return true;
