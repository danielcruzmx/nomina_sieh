...
/**
 * Clase encargada de convertir cantidades monetarias a su representacion en texto
 * @author Oscar S.
 */
public class NumerosLetras extends JRDefaultScriptlet {
    ...
    public String getPrestadorGeneroTitulo(String curp) {
        String curpIngresado = curp.toUpperCase();
        String generoCurp = curpIngresado.substring(10, 11);
        return generoCurp.equals("H") ? "EL PRESTADOR DE LOS SERVICIOS" : "LA PRESTADORA DE LOS SERVICIOS";
    }
    
    public String getPrestadorGenero(String curp) {
        String curpIngresado = curp.toUpperCase();
        String generoCurp = curpIngresado.substring(10, 11);
        return generoCurp.equals("H") ? "El Prestador de los Servicios" : "La Prestadora de los Servicios";
    }
    
    public String getCiudadanoGenero(String curp) {
        String curpIngresado = curp.toUpperCase();
        String generoCurp = curpIngresado.substring(10, 11);
        return generoCurp.equals("H") ? "EL C." : "LA C.";
    }
    
    public int getQuincenas(String fechaIni, String fechaFin) {
        final long MILLSECS_PER_DAY = 24 * 60 * 60 * 1000; //Milisegundos al día
         ...
         Date fecFin = null;
         try {
             fecIni = formatter.parse(fechaIni);
             fecFin = formatter.parse(fechaFin);
         } catch (ParseException e) {
             // TODO
         }
         
        long diferencia = (fecFin.getTime() - fecIni.getTime()) / MILLSECS_PER_DAY;
        int quincenas = (int)diferencia / 15;
        return quincenas;
    }
}
